...
public class NewtonParallel {
	...
	public static void main(String[] args) {
		// first check for passed parameters
		...
		if(args.length == 1 || args.length == 2) {
			for(String arg : args) {
				...
				if(argSplit[0].equals("--workers")) {
					if(!hasWorkers) {
						hasWorkers = true;
						
						try {
							workers = Integer.parseInt(argSplit[1]);							
						} catch(Exception e) {
							System.out.println("Wront number format. Default values are going to be used.");
							break;
						}
					} else {
						System.out.println("Cannot specify the same argument twice. Default values are going to be used.");
						break;
					}
				} else if(argSplit[0].equals("--tracks")) {
					if(!hasTracks) {
						hasTracks = true;
						
						try {
							tracks = Integer.parseInt(argSplit[1]);
						} catch(Exception e) {
							System.out.println("Wront number format. Default values are going to be used.");
							break;
						}
					...
					System.out.println("Wrong arguments. Default values are going to be used.");
					break;
				}
			}
		} else if(args.length != 0) {
			System.out.println("More than two arguments passed. Default values are going to be used.");
		}
	...
	public static class MojProducer implements IFractalProducer {
		...
		public void produce(double reMin, double reMax, double imMin, double imMax, int width, int height, long requestNo,
			...
			for(int i = 0; i < tracks; i++) {
				...
				offset = yMin * width;
				PosaoIzracuna posao = new PosaoIzracuna(reMin, reMax, imMin, imMax, width, height, yMin, yMax, m, data, cancel, offset);
				while(true) {
					try {
						queue.put(posao);
						break;
